    ...
    suspend fun analyzePalm(
        ...
        val promptText = when (analysisType) {
            ...
            "brief_char" -> """
                ...
                Please make each of Left Hand and Right Hand descriptions extremely detailed, comprehensive, and engaging, structured with the above bullet points using the actual emoji 📍.
        ...
        val report = if (isRussian) { // Начинаем генерацию русского демо-отчета
            ...
            val rightHandText = if (isCharacter) { // Если тип анализа - Характер
                if (isFull) { // Полный платный отчет по характеру
                    ...
                    "📍 Потенциал и вектор развития\nРекомендуется развивать открытость миру и доверять интуиции. Вектор развития направлен на интеллектуальные и духовные лидерские роли."
                }
            } else { // Если тип анализа - Судьба и события жизни
                if (isFull) { // Полный платный отчет по судьбе
                    ...
                    "📍 Потенциал и вектор развития\nВаш вектор устремлен в сторону финансовой независимости и построения прочных партнерских связей. Важные вехи ожидаются на рубеже 33-35 лет."
                }
            }
